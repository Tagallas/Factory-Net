...
public:
    explicit Storehouse(ElementID id, std::unique_ptr<IPackageStockpile> p = std::make_unique<PackageQueue>(PackageQueueType::FIFO))
        : id_(id),list_(std::move(p)) {}

    void push(Package&& package) {list_->push(std::move(package));}
